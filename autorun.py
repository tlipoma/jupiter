...
# Setup Logging
logging.basicConfig(level=logging.INFO, filename="auto_run_log.txt")
...
def save_weather_snapshot(station_array):
	...
	logger.info('Running autorun at: ' + time.strftime("%c"))
	logger.info('Attempting to save: ' + str(len(station_array)) + ' weather stations...')
	count = 0
	# Go through station list
	for station in station_array:
		# Pause 1 second between requests to avoide spamming
		time.sleep(1)
		count += 1
		logger.debug('Saving data for station #' + str(count) + ' of ' + str(len(station_array)) + ' - ' + station)
